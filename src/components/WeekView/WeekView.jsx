...
// Helper: Get current real date
const getCurrentRealDate = () => {
  return normalizeDate(new Date());
};

// Helper: Check if a day is in the past
...
export default function WeekView() {
  ...
  const handleSlotClick = useCallback((day, time, dayDate) => {
    ...
    if (taskDateNormalized.getTime() === currentRealDate.getTime() && isTaskDateTimeInPast(day, time, dayDate)) {
      console.warn(`Cannot add tasks to past time slots`);
      return;
    }
          ...
          <div className="min-w-[320px] space-y-3 p-3">
                ...
                {DAYS.map((day, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isPastDay && <div className="text-[6px] text-white/30 mt-0.5">Past</div>}
                    </button>
                  );
                })}
              </div>
                  ...
                  {hasMoreSlots && (
                    <div className="flex gap-2 p-2 bg-white/5 border-t border-white/10">
                      <button onClick={loadMoreSlots} className="flex-1 py-2 text-center text-[10px] text-white transition flex items-center justify-center gap-1 bg-white/10 rounded-lg">
                        <ChevronDown size={12} /> Load {LOAD_MORE_STEP} More
                      </button>
                      <button onClick={loadAllSlots} className="py-2 px-3 text-center text-[10px] text-white/60 transition flex items-center justify-center gap-1 bg-white/10 rounded-lg">
            ...
            {viewMode === 'day' && currentDayTimes.length === 0 && (
              <div className="bg-white/5 border border-white/10 rounded-xl p-6 text-center">
                ...
                <button onClick={() => setShowAddModal(true)} className="mt-2 px-3 py-1.5 rounded-lg bg-white text-black text-xs">
                  Add Task
                </button>
              </div>
            )}
  ...
  // ================================================================
  // DESKTOP RENDER - FULL ORIGINAL STYLES UNTOUCHED
  // ================================================================
  return (
    <TimeGrid>
