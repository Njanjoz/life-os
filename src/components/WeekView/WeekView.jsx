// src/components/WeekView/WeekView.jsx - FULLY OPTIMIZED FOR MOBILE
...
export default function WeekView() {
  ...
  // OPTIMIZATION: Single pass preprocessing - replaces multiple useMemo calls
  ...
  // OPTIMIZATION: Single stable windowing - no double slicing
  const visibleDaySlots = useMemo(() => {
    if (expandedSections['dayView']) {
      // Expanded: show up to visibleSlotCount slots
      return sortedDayTimes.slice(0, visibleSlotCount);
    }
    // Collapsed: show window around current time
  ...
  // Throttled toggle sort to prevent rapid re-renders
  const toggleSortOrder = useCallback(() => {
    if (renderLock) return;
    setRenderLock(true);
    setSortOrder(prev => prev === 'asc' ? 'desc' : 'asc');
    setTimeout(() => setRenderLock(false), 100);
  }, [renderLock]);

  // Throttled section toggle with requestAnimationFrame
  ...
  // Stable handler for slot clicks
  ...
  }, [isDayPast, isTimePast, getTaskAtSlot, renderLock]);
  ...
  if (renderLock && expandedSections['dayView']) {
    return (
