...
import { Play, Check, RotateCcw, Edit2, X, Clock, AlertTriangle, Battery, Zap, Calendar, AlertCircle, Ban, Award, Target, Shield, ShieldAlert } from 'lucide-react';
...
export default function TaskCell({ task, weekId, now, selectedDate, onUpdate, theme, isPast = false, viewMode = 'week' }) {
  ...
  useEffect(() => {
    if (isExpanded || showDelayWarning || showCompletionModal) {
      document.body.style.overflow = 'hidden';
      return () => { document.body.style.overflow = 'unset'; };
    }
  }, [isExpanded, showDelayWarning, showCompletionModal]);
  ...
  const handleComplete = useCallback(async () => {
    if (!isPast && localTask && localTask.status === 'active') {
      ...
      await completeTask(localTask.id, actualEnd, completionType);
      onUpdate();
      setIsExpanded(false);
    }
  }, [isPast, localTask, selectedDate, onUpdate]);
  ...
  }, [localTask, weekId, onUpdate]);

  const handleEdit = useCallback(() => {
    if (!isPast && localTask.status === 'pending') {
  ...
  const showRescheduleButton = !isCompleted;
    ...
    >
      ...
      >
        ...
        <div className="p-4 bg-slate-800/30">
          <div className="flex items-center justify-between mb-2"><span className="text-[10px] text-slate-400">Time Progress</span><span className="text-lg font-bold text-white">{Math.round(timeProgress)}%</span></div>
          <div className="h-2.5 bg-slate-700 rounded-full overflow-hidden"><div className="h-full transition-all duration-300 rounded-full" style={{ width: `${timeProgress}%`, background: isOngoing ? 'linear-gradient(90deg, #a855f7, #7c3aed)' : isCompleted && !isDishonest ? 'linear-gradient(90deg, #22c55e, #15803d)' : isCompleted && isDishonest ? 'linear-gradient(90deg, #f97316, #ea580c)' : 'linear-gradient(90deg, #ef4444, #dc2626)' }} /></div>
        ...
        <div className="p-4 space-y-2">
          ...
          {isPending && <button onClick={handleEdit} className="w-full py-2.5 rounded-lg text-[11px] font-semibold flex items-center justify-center gap-2 text-white bg-blue-600 hover:bg-blue-700 transition"><Edit2 size={14} /> Edit Task</button>}
        </div>
      ...
      <div className="relative h-full w-full">
        ...
        >
          ...
          <div className="relative z-10 p-1.5 h-full flex flex-col justify-between">
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-medium truncate text-white max-w-[60px]">{localTask.title ? localTask.title.slice(0, 8) : 'Task'}</p>
            ...
            <div className="flex items-center justify-between mt-0.5">
              ...
              {isRescheduledFrom && isPending && <span className="text-[6px] text-cyan-400 font-bold">Ready</span>}
