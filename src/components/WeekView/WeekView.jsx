...
export default function WeekView() {
  ...
  if (loading && tasks.length === 0) {
    return <div className="flex justify-center items-center h-96"><div className="animate-spin rounded-full h-8 w-8 border-2 border-purple-500 border-t-transparent"></div><p className="ml-2 text-xs text-slate-400">Loading...</p></div>;
  }

  return (
    <div className="space-y-3 max-w-full pb-20">
      {/* Top Bar */}
      <div className="bg-white/5 backdrop-blur-xl rounded-xl p-3 border border-white/10">
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex gap-1">
            ...
            <button onClick={() => setShowAddModal(true)} className="p-1.5 rounded-lg text-white bg-purple-600"><Plus size={14} /></button>
          </div>
        </div>
      </div>
      ...
      {/* Time Slots Grid */}
      <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-xl overflow-hidden">
        {/* Header */}
        <div className={`grid ${viewMode === 'day' ? 'grid-cols-1' : `grid-cols-${DAYS.length + 1}`} border-b border-white/10 bg-white/5`}>
        ...
        {(viewMode === 'day' ? currentDayTimes : timeSlots).map((time, slotIdx) => {
          ...
          return (
            <div key={time} className={`grid ${viewMode === 'day' ? 'grid-cols-1' : `grid-cols-${DAYS.length + 1}`} border-b border-white/10 relative`}>
              ...
              {viewMode === 'day' ? (
                <div className="p-0.5">
                  {(() => {
                    ...
                    return task ? (
                      <TaskCell task={task} weekId={week?.id} now={now} selectedDate={date} onUpdate={handleRefresh} theme={theme} isPast={isPast} viewMode="day" />
                    ) : canAdd ? (
                ...
                DAYS.map(day => {
                  ...
                  return (
                    <div key={`${day}-${time}`} className="p-0.5" onClick={() => canAdd && handleSlotClick(day, time, date)}>
                      {task ? (
                        <TaskCell task={task} weekId={week?.id} now={now} selectedDate={date} onUpdate={handleRefresh} theme={theme} isPast={isPast} viewMode="week" />
                      ) : canAdd ? (
